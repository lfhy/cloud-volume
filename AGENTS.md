...
- `go/s3/transfer_monitor.go` / `go/sync/executor.go` - physical snapshots carry an owning profile identity so sync cards can use `RemoteTaskStore.tasksForProfile()` without parsing `sync-...` IDs. `mount_read` snapshots are visible physical downloads: `sourcePath` is the opened object and `targetPath` carries the current byte range; `webdav_file.go` binds each read snapshot to `bucketAccess.config.ProfileID` before it reaches profile-scoped desktop/Web APIs. Desktop/Web adapters publish `phaseDetail` so `RemoteTaskDisplay` renders the object path with a `读取范围` subtitle instead of treating the range as a target path. `lib/state/remote_task_store.dart` merges these snapshots with the explicit local adapter fed by `transfer_queue_remote_tasks.dart`, purges legacy `transfer:metadata-op-*` rows from older bridge versions, and reconciles complete pages instead of retaining missing remote tasks. All visible task rows use the unified store. The old `TransferQueue` remains an execution/local compatibility producer, never a task display source.
