...
- `go/mount/metadata/worker.go` / `worker_execute.go` / `status.go` - scheduler/executor split for quiet-period journaling, retry backoff (15s→2m), remote source/target derived from Remote/Desired inode edges, pre-upload fingerprint checks, HEAD confirmation, conflict marking, and transfer snapshot IDs `metadata-op-<seq>`. A write executes against its immutable journal parent/name Remote edge, then a later rename moves it; directory moves/deletes wait for earlier descendant work. `claimDue` also waits on same-inode and prior parent work. `Drain` scans journal running entries absent from `ready_ops`; deletes purge confirmed subtrees. `remoteTarget` falls back to Desired only for never-synced deletes.
