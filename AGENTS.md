...
- `go/mount/delete_queue.go` - Separate delete queue with retries. It tracks claimed/running entries so a rename can rebase both pending and not-yet-started remote delete targets while holding `mutationMu`; the provider delete itself takes that same mutex, preventing an old-path delete from overtaking the move.
...
- `go/mount/manager.go` - Session replacement, global cleanup, and status probing all retain a session when its backend rejects Stop while keeping `mounted=true` and clearing `stopping`; this prevents another configuration from attaching to the still-live durable queue. `startMountSession` closes access/metadata queues on a `CleanupStale` or partial `Start` failure, so an unregistered failed session cannot leak a shared namespace. A successful Stop removes the session normally.
...
2. MOVE → `renamePath`: enter the common path gate, rebase/fence queued marker creates, drain matching writes, rebase pending delete targets, then move the remote source under the remote-mutation mutex. A local-only directory whose source is absent verifies the rebased destination marker and completes without sending an invalid move. Windows Cloud Files uses `enqueueRenamePath`, which additionally persists the move.
