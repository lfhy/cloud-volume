...
- `go/mount/metadata/service_read.go` / `go/mount/webdav_fs.go` / `go/mount/webdav_copy_test.go` - a Desired directory with no confirmed Remote edge is local-authoritative until its mkdir worker completes. Path resolution and WebDAV's recursive Finder `MKCOL` sequence therefore do not issue SFTP `ReadDir` against an uncreated parent (which would become HTTP 409 and the misleading macOS "invalid name" dialog); the protocol regression covers three nested directory levels.
