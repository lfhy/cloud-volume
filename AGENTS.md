...
- `LinearProgressIndicator` 只允许用于**有确定进度**的进度条（批量任务进度、下载进度、更新安装进度），不得作为不确定 loading spinner 使用。
