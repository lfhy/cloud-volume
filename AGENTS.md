...
- Full history cleanup is planned by `go/mount/metadata/tasks_compaction.go`: one reverse `Seq` walk accumulates inode/parent IDs referenced by later unsettled operations, checks an applied write's `ContentRef`, then deletes only safe terminal rows through the common `deleteTaskHistoryOp` index cleanup. This is intentionally equivalent to `canCompactTaskOp`'s per-row guard, which remains the precise selected-row path in `tasks_history.go`; `tasks_compaction_test.go` pins the later-parent dependency and a large terminal batch. While a cleanup request is pending, `TransfersPage._historyCleanupScope` renders a compact `CircularProgressIndicator` only inside the matching clear button; `transfers_page_remote_filters.dart` keeps the extracted filter controls below the per-file size cap.
