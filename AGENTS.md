...
- `go/mount/metadata/worker.go` / `status.go` - journal worker with quiet-period scheduling, retry backoff (15s→2m), remote source/target derived from Remote/Desired inode edges, HEAD verification before confirmation, conflict marking, transfer snapshot IDs `metadata-op-<seq>`, plus `Status`/`Reset` (reset guard refuses to clear pending or running ops unless forced). `claimDue` enforces dependency ordering via `blocked()`/`pendingOpForInode` (`inode_ops` index, earlier-seq pending/running ops only): every operation waits for earlier work on its inode; write/mkdir also wait on unfinished parent mkdir/rename. `Drain` also scans journal and waits for running operations absent from `ready_ops`, rather than reporting a false-empty queue. It recovers `OpStateRunning` ops back to pending after a crash. `executeOp` OpDelete waits for pending descendant ops (`pendingDescendantOp`), then purges the deleted directory's subtree after remote confirmation. `remoteTarget` falls back to the desired path when Remote edges are unresolvable or RemoteName is empty (never-synced deletes); `worker_test.go` / `worker_drain_test.go` pin generation ordering, reset coordination, and in-flight drain behavior.
