...
- **Known P2 (review 2026-08-17):** per-page Acquire/Release churns worker/db lifecycle (worker restarts between pages; `metadata_namespace_status` creates namespaces as a side effect); metadata `ListPage` returns direct children only, unlike the old S3 flat-prefix listing that included deep keys; a prefix pointing at a file errors instead of listing. Before M3 makes mount and page callers concurrent, make shared Service quiet/read-only configuration immutable or synchronize it, and add a provider-level `MaterializeDirectory` RootPrefix assertion rather than only checking the scoped backend wrapper. Move handle lifetime to page-session scope later.
...
- `go/mount/bucket_access.go` - `bucketAccess` owns all queues. `newBucketAccess` creates `dirSync` (`newDirSyncQueue`), `writeback` (`newWritebackQueue`), `deletes`. `close()` shuts down queues; `drainWritebackContext` drains file writeback without discarding persisted entries when its caller times out. `release()` shuts down `dirSync` only.
...
- `go/mount/writeback_restore.go` - `restorePersistedEntries` filters records via `canRestoreRecord`: local path must exist, be a file, and be inside either `sessionRoot` or `cacheRoot`, so the normal `~/.cloud-volume/cache/mounts/<bucket>/...` writeback source survives remount. `restorePersistedMutations` rebuilds barriers/rebases and replays rename records **without the `run` closure**.
- `go/mount/dir_sync_queue.go` - Directory marker create queue (memory-only, no persistence). `stageLocalDirectory` → `queueRemoteDirectory` → `enqueue`. 2 workers; `flush` calls `backend.CreateDirectory` and still **swallows errors** (log only). `rebaseAndFence` is used by both queued Cloud Files renames and synchronous `renamePath` calls.
- `go/mount/bucket_access_writes.go` - `createDirectory` only stages locally + enqueues marker. `renamePath` (macOS WebDAV, Linux FUSE, WinFsp) first rebases/fences matching directory markers, waits for them, then rekeys pending uploads and either recognizes the newly-created destination marker or moves the confirmed remote source. `enqueueRenamePath` (Windows Cloud Files only) captures a persisted `mutationRecord` with barrier generation and a legacy `run` closure that creates the renamed directory remotely when the old path is absent.
...
2. MOVE → `renamePath`: rebase/fence queued marker creates before touching remote state, then re-key any pending writeback entry. A local-only directory whose source is absent verifies the rebased destination marker and completes without sending an invalid move. Windows Cloud Files uses `enqueueRenamePath`, which additionally persists the move.
...
- **Directory creates are not persisted at all**; `dirSync.flush` still swallows errors and needs its own observable pending/error state. The rebase fix prevents legacy synchronous renames from creating the old marker name, but a crash before the marker completes still loses that memory-only create.
...
- **Page-vs-mount coupling details (2026-08-17 review):** Flutter `listObjectPage` uses the mount session view only when the bucket is mounted (`bridge/dispatch_paging.go` → `mount.ListMountedObjectPage`, `go/mount/object_page.go`); otherwise it calls the backend directly. Out-of-mount UI writes succeed remotely first and then patch mount caches via `NotifyExternal*` (`go/mount/external_invalidation.go`). `InvalidateExternalUpload` now preserves a matching pending writeback file while invalidating remote lookup caches, but this remains a legacy repair rather than the unified journal ingest path. Page paging over mounts uses process-local snapshot tokens (`go/mount/object_page_snapshots.go`, 2-minute TTL, max 16). The unified-view refactor must decouple metadata storage from mount-session lifetime and route UI/mount/P2P/poll writes through one journal entry point.
