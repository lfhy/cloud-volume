...
- **Gotcha:** `DefaultManager` is shared, but per-page `Acquire/Release` still closes a service when no mount retains the same namespace. M3 must make mounts hold a manager handle for their lifecycle before relying on shared page/mount metadata state.
