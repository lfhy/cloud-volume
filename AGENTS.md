...
- Development cache recovery: namespace bbolt state is under `~/.cloud-volume/runtime/metadata/v1/<namespace>/metadata.db`; its adjacent `namespace.json` maps the opaque namespace ID to profile/bucket, while chunk state is under the configured cache root at `metadata-chunks/<namespace>`. For a fresh-development stale metadata view with no pending journal/content, rebuild or move both namespace roots rather than adding a migration for cache-only fields. Never remove a namespace that still has pending operations or protected chunks.
