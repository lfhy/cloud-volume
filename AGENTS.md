...
- **M3 read-integration constraints (exploration 2026-08-17):** a mount must retain its own `metadata.AcquireHandle` until the real platform session stops, while missing `ProfileID` alone keeps the legacy fallback; any other metadata acquire failure must fail mount startup instead of silently forking views. Metadata is the remote base only: reads must merge system overlay first, tombstones before remote materialization, then local files/directories, queued or restored writeback, and `dirSync` entries. The mount backend is deliberately unscoped, so metadata materialization must use its scoped provider. Stable OID projection is deferred to M5 while legacy-only writes still bypass Desired+journal.
...
- `go/mount/bucket_access.go` - `bucketAccess` owns all queues. `newBucketAccess` creates `dirSync` (`newDirSyncQueue`), `writeback` (`newWritebackQueue`), `deletes`. `writebackMu` makes local cache staging plus durable queueing atomic with external-upload invalidation. `close()` shuts down queues; `drainWritebackContext` drains file writeback without discarding persisted entries when its caller times out. `release()` shuts down `dirSync` only.
- `go/mount/writeback_queue.go` / `writeback_queue_drain.go` - File upload queue. `stageLocalWrite` (from WebDAV close, FUSE/WinFsp publish, or Windows watcher) stores the local marker and calls `enqueue` under `writebackMu`; the queue delays by `WritebackQuietSeconds` and drains through a cancellation-safe dispatcher. If a cancellation hits queue backpressure, every not-yet-sent entry is reset runnable rather than stranded `queued=true`. `flushNow` stats the resolved local path, refreshes if size/mtime changed, `UploadFile`, then HEAD-verify, cache update, peer broadcast, store delete. Missing source = success-with-cleanup (`flush-missing`), so an upload whose cache file vanished is silently dropped.
- `go/mount/writeback_store.go` - Per-PID JSON store plus global registry keyed by store dir. Every writeback record has a hash scope of `ProfileID/storage type/endpoint/bucket/RootPrefix/access key id`; store merge keys are `scope + path`, so same virtual paths from different remotes survive compaction independently. An active in-process queue rejects attaching a changed scope.
...
- `go/mount/bucket_access_writes.go` - `createDirectory` only stages locally + enqueues marker. `stageLocalWrite` protects the marker-to-queue handoff. `renamePath` (macOS WebDAV, Linux FUSE, WinFsp) first rebases/fences matching directory markers, waits for them, then rekeys pending uploads, moves their physical cache files, and either recognizes the newly-created destination marker or moves the confirmed remote source. `enqueueRenamePath` (Windows Cloud Files only) captures a scoped persisted `mutationRecord` with barrier generation and a legacy `run` closure that creates the renamed directory remotely when the old path is absent.
