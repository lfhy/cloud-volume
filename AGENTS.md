...
**Status (2026-08-17):** the first-stage persistent inode metadata core has landed under `go/mount/metadata`, but the legacy queue/mount/page read paths have not yet been switched over. Until the migration batches complete, this entry still describes the live legacy behavior plus the new package's role.

#### New persistent metadata core

- `go/mount/metadata/types.go` - durable contracts: `Inode`, `Dirent`, `Op`, `ListingState`, `ContentRef`, `Object/Page/Cursor`, `Namespace`, and the narrow provider `Backend` interface. `ContentRef` holds ordered SHA-256 chunk hashes, never a local path; `Namespace.CacheRoot` separates chunk data from the runtime metadata DB. Inode identity is local to each namespace; no symlinks/hardlinks.
- `go/mount/metadata/store.go` / `store_io.go` - bbolt schema v2 with `schema/inodes/dirents/journal/listing_state/content_refs/chunks/ready_ops/inode_ops`. Root inode is fixed at 1; inodes are monotonically allocated and never reused. Every write is serialized; schema mismatch returns an error (reset/rebuild, no upgrade).
- `go/mount/metadata/service_read.go` - `MaterializeDirectory`, `ListPage`, `StatPath`, `StatInode`, `Resolve`, `Path`. Paging cursors are base64url `{v,inode,revision,lastNameKey}`; a changed directory revision returns `ErrStaleCursor` so the UI reloads rather than pretending snapshot consistency.
- `go/mount/metadata/write.go` - Desired-tree writes: `CreateDirectory`, `StageWrite` (fixed 4 MiB chunking, SHA-256, fsync plus atomic rename before bbolt ref commit), `Write`, `Rename`, `Delete`. Rename changes exactly two dirent B+Trees plus one inode edge; descendant inodes and content files are untouched. It rejects parent cycles, directory overwrites, and collisions.
- `go/mount/metadata/chunk_store.go` / `chunk_protection.go` - pending data plane: chunks live at `<CacheDirectory>/metadata-chunks/<namespace>/chunks/<hash[:2]>/<hash>`; `chunks[hash]` has logical `nlink`, size, and last-access values. StageWrite writes/syncs chunks before bbolt, deduplicates equal blocks, and publishes a conservative atomic `protection.json` before adding refs. Worker uploads splice a temporary full file; retirement/deletion decrements refs and removes zero-link blocks. Startup sweep removes uncommitted orphan chunks and interrupted splice files.
- `Op.ContentGeneration` binds every write journal record to its exact `ContentRef`, so rapid successive writes retire their own generations rather than uploading the newest staged ref twice and leaving a stuck later op.
...
- `go/mount/metadata/status.go` additionally counts `PendingContent`; the reset guard refuses to rebuild even when staged chunks exist without a journal op, and forced reset sweeps the separated chunk root.
- `go/mount/metadata/worker.go` / `status.go` - journal worker with quiet-period scheduling, retry backoff (15s→2m), remote source/target derived from Remote/Desired inode edges, HEAD verification before confirmation, conflict marking, transfer snapshot IDs `metadata-op-<seq>`, plus `Status`/`Reset` (reset guard refuses to clear pending ops unless forced). `claimDue` enforces dependency ordering via `blocked()`/`pendingOpForInode` (`inode_ops` index, earlier-seq pending/running ops only): write/mkdir wait on unfinished parent mkdir/rename and earlier same-inode ops; it also recovers `OpStateRunning` ops back to pending after a crash. `executeOp` OpDelete waits for pending descendant ops (`pendingDescendantOp`), then purges the deleted directory's subtree after remote confirmation. `remoteTarget` falls back to the desired path when Remote edges are unresolvable or RemoteName is empty (never-synced deletes).
...
#### M2: page read path via metadata (2026-08-17)

- `bridge/dispatch_metadata.go` (new) - routes `list_object_page` through the unified inode view: `listObjectPageFromMetadata` acquires the namespace via `metadata.DefaultManager().Acquire` (fallback to legacy path on `ErrNoProfileID`), resolves the prefix to a directory inode via `StatPath` (ancestor materialization), calls `ListPage`, and adapts to the `objectInfoWire`/`objectPageWire` JSON shape (`key/size/lastModified/etag/isDir/nextToken`) that Dart's `ObjectListPage` already parses. `forceRefresh` forces `MaterializeDirectory` so manual refresh bypasses the materialized-view cache. `ErrStaleCursor` is consumed Go-side (`listPageWithStaleRestart`): pagination transparently restarts from page one until Dart learns stale-cursor reload. Also exposes `metadata_namespace_status` returning `Service.Status()`. `metadataListFunc` indirection lets tests substitute manager/backend wiring.
- `bridge/dispatch_paging.go` - `listObjectPage` order is now: metadata namespace → `ListMountedObjectPage` (mount-session cache) → provider-direct. `forceRefresh` still invalidates the mount list cache first.
...
- **Gotcha:** Flutter configs don't yet carry `profileId` (Go `loadProfileFromDB` backfills it, but Dart `toJson` doesn't include it), so in practice most desktop page listings still take the fallback until the Dart model forwards `profileId`. That wiring is the remaining piece of full M2 enablement.
- **Known P2 (review 2026-08-17):** per-page Acquire/Release churns worker/db lifecycle (worker restarts between pages; `metadata_namespace_status` creates namespaces as a side effect); metadata `ListPage` returns direct children only, unlike the old S3 flat-prefix listing that included deep keys; a prefix pointing at a file errors instead of listing. Move handle lifetime to page-session scope later.
...
#### Legacy queues (still live until migration batches land)
...
- `go/mount/bucket_access.go` - `bucketAccess` owns all queues. `newBucketAccess` creates `dirSync` (`newDirSyncQueue`), `writeback` (`newWritebackQueue`), `deletes`. `close()` shuts down queues; `drainWriteback()` only drains file writeback. `release()` shuts down `dirSync` only.
- `go/mount/writeback_queue.go` - File upload queue. `scheduleUpload` (from WebDAV `writableWebDAVFile.Close` or Windows watcher) → `enqueue` keyed by virtual path, quiet-period timer (default 10s; `WritebackQuietSeconds`), persisted per-PID to `sessionRoot/writeback/queue-<pid>.json`. `flushNow` stats the resolved local path, refreshes if size/mtime changed, `UploadFile`, then HEAD-verify, cache update, peer broadcast, store delete. Missing source = success-with-cleanup (`flush-missing`), so an upload whose cache file vanished is silently dropped.
- `go/mount/writeback_store.go` - Per-PID JSON store plus global registry keyed by store dir; queues can be re-acquired across sessions. `acquireWritebackQueue` restores persisted mutations before dispatchers start.
- `go/mount/writeback_restore.go` - `restorePersistedEntries` filters records via `canRestoreRecord`: local path must exist, be a file, and be **inside `sessionRoot`**. Default `cacheRoot` is `~/.cloud-volume/cache/mounts/<bucket>/...` while `sessionRoot` is `~/.cloud-volume/runtime/mounts/<bucket>/...`, so ordinary macOS/WebDAV writeback entries are **discarded on restart**. `restorePersistedMutations` rebuilds barriers/rebases and replays rename records **without the `run` closure**.
- `go/mount/dir_sync_queue.go` - Directory marker create queue (memory-only, no persistence). `stageLocalDirectory` → `queueRemoteDirectory` → `enqueue`. 2 workers; `flush` calls `backend.CreateDirectory` and **swallows errors** (log only). `rebaseAndFence` is only called from `enqueueRenamePath` (Windows Cloud Files path), not from `renamePath`.
- `go/mount/bucket_access_writes.go` - `createDirectory` only stages locally + enqueues marker. `renamePath` (macOS WebDAV, Linux FUSE, WinFsp) renames pending writeback entries, then if any existed probes remote source and returns early when absent (local-only rename succeeds without creating remote target). It does NOT rebase/fence `dirSync`. `enqueueRenamePath` (Windows Cloud Files only) captures a persisted `mutationRecord` with barrier generation and a legacy `run` closure that creates the renamed directory remotely when the old path is absent.
...
- `go/mount/delete_queue.go` - Separate delete queue with retries.
- `go/mount/webdav_fs.go` / `go/mount/webdav_file.go` - Finder path: MKCOL → `createDirectory` (local + dirSync), MOVE → `webDAVFS.Rename` → `renamePath`; PUT close → staged temp renamed into hashed cache path → `registerLocalWrite` + `scheduleUpload`.
- `go/mount/macos_mount.go` - macOS `mountSession.stop` unmounts, stops WebDAV server, then `access.close()`. It does **not** call `drainWriteback()` (Linux FUSE and Windows WinFsp do), so quitting/unmounting macOS discards not-yet-flushed uploads instead of draining them.
...
2. MOVE → `renamePath`: if a writeback entry exists under old prefix, re-key it and probe remote; source absent ⇒ success, but the queued marker is not rebased (macOS/Linux/WinFsp) so the remote may later get the OLD directory name. Windows Cloud Files uses `enqueueRenamePath`, which rebases/fences and persists the move.
3. PUT close → hashed cache file + `writeback.enqueue("正确名/文件")` (visible as `mount-writeback-*` pending task, `sync_wait` → `upload_wait`).
...
- **Restore filter drops uploads:** `canRestoreRecord` requires `isPathWithin(localPath, sessionRoot)` but default cache lives outside sessionRoot; after restart persisted upload entries are filtered out and merged away. Combined with macOS's no-drain stop, a quit inside the quiet window silently loses both the file upload and the directory marker.
- **Directory creates are not persisted at all**; `dirSync.flush` swallows errors, and queued entries are only rebased by the Windows `enqueueRenamePath` path. macOS/WebDAV rename of a local-only directory can create the default-name marker remotely after the user renamed it.
...
- **Page-vs-mount coupling details (2026-08-17 review):** Flutter `listObjectPage` uses the mount session view only when the bucket is mounted (`bridge/dispatch_paging.go` → `mount.ListMountedObjectPage`, `go/mount/object_page.go`); otherwise it calls the backend directly. Out-of-mount UI writes succeed remotely first and then patch mount caches via `NotifyExternal*` (`go/mount/external_invalidation.go`); `InvalidateExternalUpload` removes local staged data via `cache.removeLocalPath`, which can delete pending mount writes. Page paging over mounts uses process-local snapshot tokens (`go/mount/object_page_snapshots.go`, 2-minute TTL, max 16). The unified-view refactor must decouple metadata storage from mount-session lifetime and route UI/mount/P2P/poll writes through one journal entry point.
...
- Transfer UI shows only `mount-writeback-*` tasks; directory marker creates and mutation retry errors are log/mount-status only.
