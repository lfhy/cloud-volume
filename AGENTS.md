...
- `go/mount/macos_mount.go` / `macos_mount_test.go` / `macos_command.go` / `macos_command_test.go` / `system_mounts.go` - Default mounts use the synchronous `mount_webdav` path; Finder `open` is asynchronous and guarded by a per-path single-flight gate because a process blocked in a WebDAV filesystem call may ignore `CommandContext` cancellation for much longer than five seconds. The mount command is never canceled merely because a mount-table row appears; URL/path probing remains for recovery and stale-volume checks. Stop order is system unmount first, local WebDAV/access shutdown second; an unmount failure keeps the server and session alive for retry. Tests cover command/probe behavior, Finder-open coalescing, decoded/requested path matching, fallback order, and server survival after an unmount error.
...
- `/sbin/mount_webdav` is synchronous: mount startup waits for the command to return instead of canceling it when the mount table row first appears. Probe/recovery is only used after an error, and the command's own result remains authoritative.
