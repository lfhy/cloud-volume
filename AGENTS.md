...
### Loading indicators (binding — reference: file manager page)

All loading spinners MUST use `AppLoadingIndicator` (`lib/widgets/app_loading_indicator.dart`, shadcn-style arc spinner over an 18%-alpha track). Raw Material `CircularProgressIndicator` is **forbidden** anywhere under `lib/` — it has a different head style, no track, and default 4px stroke that breaks visual rhythm.

Size tiers (the file manager page is the reference; copy, don't reinvent):

| 场景 | 规格 | 参考实现 |
|---|---|---|
| 页面主体加载 | `Center > Column(min) > AppLoadingIndicator(size: 22, strokeWidth: 2.4)` + 12px gap + 阶段文案（`textTheme.p` w600）+ 可选 muted 13px detail | `lib/pages/file_manager_page.dart` `_buildLoadingView`、任务队列 `_RemoteInitialLoading` |
| 按钮/动作内联 | `AppLoadingIndicator(size: 14, strokeWidth: 2)` + 7px gap + 动作词；实心按钮传 `color: primaryForeground` | `lib/pages/login_page.dart`、`transfers_page.dart` `_batchActionButtonChild` |
...
- `LinearProgressIndicator` 只允许用于**进度条**——有确定进度（批量任务、下载、配额），或总量未知但确属进度场景（更新安装 `value: null`）；不得作为通用 loading spinner 使用。
...
- The poll loop in `RemoteTaskStore` MUST request `includeHistory: false` so every active task fits on page one regardless of accumulated done history. History is fetched only by the page-entry `loadInitialHistory()` flow (which calls `loadMore` up to its first 100 rows) or a user-initiated later `loadMore`; all of those requests set `includeHistory: true`.
...
### Debug task-state endpoint (binding)

`make run` exports `CV_DEBUG_ADDR` (default `127.0.0.1:8765`); the bridge lazily starts a localhost-only HTTP listener (`bridge/debug_tasks.go`) on the first bridge call. Endpoints:
- `GET /debug/tasks` — every namespace's live task projection: `id/state/status/lastError/retry/nextRetryAt/seq/createdAt/updatedAt`. Use this first when tasks appear stuck (e.g. 验证远端 never resolving) — `lastError` + `retry` distinguishes "probe keeps failing and retrying" from "worker stopped".
- `GET /debug/transfers` — runtime transfer monitor snapshots behind metadata tasks (`id/type/status/detail/bucket/key/bytes`).
- `WS /debug/task-events` — push channel. Metadata workers call `Service.NotifyTaskChanged()` on every op state transition (`worker_execute.go`); `Manager.SubscribeTaskChanges()` dynamically fans existing and newly acquired namespaces into one channel and the socket sends one `{"event":"changed"}` tick per coalesced signal. Desktop `RemoteTaskStore` (`lib/state/remote_task_events.dart`) connects via `dart:io` WebSocket on bind, triggers `pollActive()` on each tick, and silently reconnects with 2s backoff; `remote_task_events_web.dart` deliberately uses polling because the debug listener is localhost-only. Polling (700ms active / 2s idle) remains the correctness fallback, so the push path is an optimization, never a data source. Client dispose closes its WebSocket; the server reads for peer close and unsubscribes immediately. `make run` also passes the address as `--dart-define=CV_DEBUG_ADDR=...`.
  Notify coverage (binding): every durable op-state transition must call `NotifyTaskChanged()` — worker claim/execute, reconciliation success/failure/cancel, user Cancel/Retry/Trigger, startup `recoverDurableStates`, journal admission (`remoteMutation.finish`), and both clear-history paths. New call sites that mutate op state without notifying are bugs. `Service.groupCacheInvalidate` (installed by Manager) drops the 250ms group memo on each notify so a push-triggered poll reads fresh data.
Set `CV_DEBUG_ADDR=` (empty) to disable the listener. Binding on 0.0.0.0 or exposing it off-localhost is forbidden; it is a diagnostics-only surface.
...
- `RemoteTaskStore.pollActive` uses `includeHistory: false` so active tasks always fit on the first page. `TransfersPage` calls `loadInitialHistory()` only when its `IndexedStack` child becomes active; it joins the serialized in-flight read, then calls `loadMore` until the first **100** terminal rows are present. Later history is user-paged with `loadMore`; every history request passes `includeHistory: true`. Any new polling caller must preserve this separation.
...
- `bridge/dispatch_remote_tasks.go` + `bridge/remote_task_visibility.go` and the Web equivalents separate **count scope** from **response visibility**: `remoteTaskCountInput` / `webRemoteTaskCountInput` always include terminal history when computing `total` and `queue`; `remoteTaskResponseItems` / `webRemoteTaskResponseItems` hide only terminal rows for an active-only poll. Active-only uses the actual active-row length as its page limit because `*TaskPageSlice(..., 0)` means the normal 100-row default. `remote_task_visibility_test.go` pins 200 history + 101 waiting → counts 301/200/101 while all 101 active rows return.
- `RemoteTaskStore.tasks` may use a local producer only while it is active; terminal task-page history must originate from Go. `TransferQueue` now removes terminal local projections and persists only active producer hand-offs; on restart it drops any old v2 payload because a Dart producer cannot resume. This fixes the diagnostic signature `backend tasks=0, transfers=0` with a stale `历史 200` list.
- `Manager.ListTaskGroups` caches task groups for 250ms, but the memo is versioned and invalidated on every durable task transition **and** namespace acquire/release/removal. A cache build that races either kind of change retries rather than publishing a transient empty/nonempty projection; otherwise the bridge can alternate between a real history and `total=0` as page handles are opened and released. `TestTaskGroupCacheTracksNamespaceAcquireAndRelease` pins that topology invariant.
- When a `RemoteTaskPage` explicitly reports `total=0`, `RemoteTaskStore` evicts every cached Go row even during an active-only poll; this is the one active-only response that authoritatively describes the entire projection as empty. Terminal Dart producer snapshots are retained only in `executionTask` so batch dialogs can render their final state without reappearing in the task list. `remote_task_store_test.dart` and `batch_task_progress_dialog_test.dart` pin both paths.
- `RemoteTaskStore.canLoadMoreHistory` exposes the first history request even though active-only polling has no history cursor. `TransfersPage` must render the history pager before its empty/filter-empty states, otherwise a history-only queue is unreachable. Since 2026-08-29 (user-directed) `_RemoteHistoryPager` is the **last `ListView` child**, not a fixed card footer: it renders `历史已显示 loaded / total` plus a `加载下一页（还剩 n 条）` button (busy state = centered `正在加载历史…` spinner row) and therefore comes into view only when the user scrolls to the final row; a full first page keeps it below the fold by design. The `showHistoryPager` early-return logic is what still guarantees reachability — a history-only queue skips the empty states and renders the card whose list ends with the pager row (or `_RemoteInitialLoading` while the first page reads). The WebSocket only signals a fresh active-only poll (counts/active rows); it deliberately does not stream unbounded terminal history, so this explicit pager is authoritative for later history. `_applyQueue` must invalidate both `_nextCursor` and `_hasLoadedMore` whenever an authoritative active-only poll reports a larger history total: offset pagination shifts when a new terminal row sorts in front, and merely reopening an exhausted flag would skip the second later completion. A cached row is removable history only for `done`/`canceled`; `failed`/`conflict` remain in the active-only response and retry queue until they disappear remotely. Desktop and Web queue classifiers normalize runtime `pending` as waiting and `completed`/`cancelled` as history to match Dart wire parsing.
- `TransfersPage` lives in an `IndexedStack`: it MUST schedule `loadInitialHistory()` in a post-frame callback, never synchronously from `didUpdateWidget`, because its task-store notification also reaches sidebar animation listeners during the parent build. `SidebarTransferStatus._syncAnimation` independently defers AnimationController updates from `SchedulerPhase.persistentCallbacks` as a safety net. The header's `清理全部历史 <server count>` action is always shown when `queue.history > 0`, including while selection is active or only the first history page is cached; it calls `clearHistory()` with no IDs, whereas `清理历史 <n>` remains the selected-rows action. `transfers_page_batch_actions_test.dart` pins both paths and the build-phase animation regression.
- Full history cleanup is planned by `go/mount/metadata/tasks_compaction.go`: one reverse `Seq` walk accumulates inode/parent IDs referenced by later unsettled operations, checks an applied write's `ContentRef`, then deletes only safe terminal rows through the common `deleteTaskHistoryOp` index cleanup. This is intentionally equivalent to `canCompactTaskOp`'s per-row guard, which remains the precise selected-row path in `tasks_history.go`; `tasks_compaction_test.go` pins the later-parent dependency and a large terminal batch. While a cleanup request is pending, `_RemoteBatchAction` renders a compact `CircularProgressIndicator` only inside the matching clear button; `transfers_page_remote_filters.dart` keeps the extracted filter controls below the per-file size cap.
- Queue-level `立即同步` follows `lib/pages/transfers_page_remote_header.dart` → `RemoteTaskStore.triggerAllRemoteTasks` → desktop `trigger_all_remote_tasks` / Web `trigger_all_remote_tasks` → `Manager.TriggerPendingTasksFor` → `Service.TriggerPendingTasks`. It handles all in-scope **metadata** pending groups in one backend call, sets only `NextAttemptUnixNano=now` and one-shot `SkipQuiet`, then emits one task-change tick and wakes the worker. Web calls require a nonempty active ProfileID (`webTaskSyncProfileID`), never falling back to Manager's all-namespace empty scope; rejected/error actions return non-2xx so Dart does not report a false zero-task success. It deliberately bypasses a retry deadline, but MUST NOT execute provider work directly or weaken `Worker.claimDue`'s dependency checks: same-inode sequencing, parent/move barriers, recovery/cancel handling, and one-at-a-time provider execution remain authoritative. `tasks_sync_test.go` pins both parent→child and mkdir→rename bulk triggers, where the later op is not claimed until its prerequisite is terminal; `remote_tasks_test.go` pins the empty-ProfileID rejection; `transfers_page_batch_actions_test.dart` pins one frontend call plus its scoped spinner. `_RemoteBatchAction` also ensures the clear/sync spinner appears only on the action actually in flight.
- `lib/state/remote_task_store_polling.dart` owns one serialized `_inFlightRead` for active polls and history refreshes. `loadMore` must join then retry behind that gate rather than treating another page's `refresh()` as completion; do not restore timer-based busy-waiting or parallel reads, which expose a transient manual-load button and can corrupt a cursor. `loadInitialHistory` walks active-prefix pages at normal size, then passes the remaining terminal-row capacity as `limit`, so queues of at most 100 terminal rows render in full while larger queues use one bounded first page; `isLoadingMoreHistory` owns the fixed pager spinner while its request waits behind an active poll. `_applyQueue` advances the non-destructive `_historyCursorVersion` on terminal-count growth; a continuation response that itself detects the growth is discarded before merge because its offset has already shifted, then a user page request retries page one behind the same read gate **once**. Page-entry loading instead yields that shift to its outer remaining-row loop, so a bounded first response never appends a full second page; a second shift yields back to the visible pager rather than spinning forever. `remote_task_store_cleanup.dart` alone advances destructive `_historyEpoch`, waits an older read, and polls fresh before touching local caches; stale clear/history responses must never reset a new binding or resurrect deleted rows. `remote_task_store_test.dart` pins 100/150 page progression, two WebSocket-style history growth events after cursor exhaustion, an initial-page growth that remains capped at 100 rows, repeated in-flight continuation growth, and continual-growth liveness, as well as serialized reads, retry after first-page failure, cursor invalidation, clear/history races, and API rebinding; `transfers_page_batch_actions_test.dart` pins the visible fixed pager and inactive→active `IndexedStack` transition.
- Task-change subscriptions are lifecycle-bound: `Service.Close()` closes its listeners, and `Manager.SubscribeTaskChanges()` removes each service registration when that channel closes. This is required because list polling opens and releases history-only namespaces every cycle; retaining the closure would leak a closed service and fan-in goroutine for the lifetime of a WebSocket client. Browser builds select `remote_task_events_web.dart` rather than importing `dart:io`, preserving the polling fallback.

**Known P1/P2 (review 2026-08-22, follow-up 2026-08-24):**

- P1 — FIXED (2026-08-22): `pollActive` now shares the `_polling`/generation/API-identity guards with `refresh()` and updates `_total`/`_queue`/`_freshness`/`_capabilities` every poll; a real all-zero queue report is applied via the new `queue.reported` / `page.hasTotal` presence flags (`lib/models/remote_task_wire.dart`) instead of `total > 0` truthiness.
- P1 — FIXED (follow-up): active-only polling now returns full-scope `total`/queue counts (including history) while filtering only response rows. This fixed the observed `共 0 项 · 已加载 200` / `历史 0` UI state. Both desktop and Web use the active result length as the unbounded page limit; the former `limit=0` fix was ineffective because the shared slicer interprets zero as its 100-row default.
- P1 — FIXED (follow-up): `Manager.ListTaskGroups` now invalidates and versions its 250ms memo for namespace topology as well as task-state mutations. This closes the cache race that could still serve an empty group snapshot immediately after `warmKnownTaskNamespaces` reopened retained history.
- P1 — FIXED (follow-up): queue classifier parity now maps runtime `pending` to Waiting and `completed`/`cancelled` to History on both desktop/Web. Failed/conflict rows are unsettled (not clearable history), so active polling purges an expired failed runtime row and clear-history never deletes failures from only one source.
- P1 — FIXED (follow-up): retained history is loadable from the first active-only page through `canLoadMoreHistory`, including an otherwise empty history tab. Dynamic task-change fan-in now releases per-service subscriptions when a warmed namespace closes, preventing polling cycles from retaining closed services/goroutines.
- P1 — FIXED: `NotifyTaskChanged` coverage completed — worker claim (pending↔running), `recoverDurableStates`, reconcile success/failure/cancel-finalize, user Cancel/Retry/Trigger (`tasks_control.go`), journal-commit admission (`remoteMutation.finish`), and both clear-history paths. `NotifyTaskChanged` also drops the manager group cache via a `Service.groupCacheInvalidate` hook so push-triggered polls never read the 250ms memo.
- P1 — FIXED: `Manager.SubscribeTaskChanges` installs its dynamic hook and subscribes existing services under the same `m.mu → taskChangeMu` order as `Acquire`, closing the late-namespace registration race. Service unsubscribe closes its channel, so manager fan-in goroutines exit.
- P2 — FIXED: `TaskEventsHandle.dispose` now closes the live WebSocket and `bridge/debug_tasks.go` detects peer close to unsubscribe server-side. The debug listener rejects every non-numeric-loopback address. `remote_task_store` was split into parts (`remote_task_store_merge.dart`, `remote_task_store_cleanup.dart`, `remote_task_store_local.dart`) and `RemoteTask` wire classes moved to `lib/models/remote_task_wire.dart` to satisfy the 500-line cap. Both `ClearTaskHistory` flavors now report distinct task groups and notify subscribers.
...
- `go/mount/metadata/worker.go` / `worker_execute.go` / `worker_move.go` / `worker_reconcile.go` / `status.go` - scheduler/executor split for quiet-period journaling, retry backoff (15s→2m), pre-upload fingerprint checks, bounded provider confirmation (45s deadline), confirmation, conflict marking, namespace-qualified transfer snapshot IDs `metadata-op-<namespace>-<seq>`, and probe-based cancellation reconciliation. Verifying/cancel-requested/reconciling states are indexed and recovered on startup; a post-side-effect verification error keeps the op in `verifying` for probe reconciliation instead of replaying an overwrite against its old fingerprint. A cancel only becomes terminal after provider absence or an explicit compensating move/delete is verified. A write executes against its immutable journal parent/name Remote edge, then a later rename moves it; move source/target and confirmation parent/name are frozen before the provider call, so a later Desired rename cannot redirect replay. Directory moves/deletes wait for earlier descendant work. Complete worker claim-and-execute passes are serialized between the background loop and `Drain`, preserving replacement delete-before-move ordering; `Drain` uses a nonblocking pass-gate attempt so its context remains responsive while polling work is active. `claimDue` also waits on the shared unsettled-state predicate for same-inode and prior parent work and a namespace-wide local-mutation quiet barrier, so recursive Finder copies remain local while new journal operations arrive. `Drain` bypasses only that transient barrier, while retry deadlines and dependencies remain enforced. `SkipQuiet` is a durable one-shot flag for manual trigger/retry and is cleared when claimed; pending timestamps rebuild the barrier after restart. Deletes purge confirmed subtrees. `remoteTarget` falls back to Desired only for never-synced deletes.
- `go/mount/metadata/status.go` / `worker_move.go` / `tasks_control_rollback.go` - confirmation persists a non-empty provider `HeadObject.LastModified` into both `Inode.RemoteMTime` and the visible `Inode.MTime`, except while a later unsettled write owns a newer local timestamp. Pending materialization preserves the remote value separately, and cancellation restores it atomically after releasing a newer staged write. This keeps a newly created SFTP directory's timestamp visible immediately after sync without letting an older confirmation overwrite a rapid later write or a later cancellation retain the discarded local time; `worker_mtime_test.go` pins all three cases and `sftp_backend_test.go` pins the actual SFTP `Stat` source. Directory materialization remains the listing-path source of remote timestamps, and the Dart browser intentionally leaves only genuinely empty values blank.
- `go/mount/manager.go`, `object_page.go`, `system_mounts.go`, and `macos_mount_stop.go` - macOS status polling probes `mount -t webdav`; a transient probe error keeps either a live session or a retained partial-start session reachable for explicit cleanup and records a diagnostic, while a confirmed missing mount records an unexpected-disconnect error before releasing the local server/access. A retained partial attempt is deliberately not reusable for mount/open/page reads, preventing a false mounted success while its system state is unknown. macOS `webdavfs_agent` can independently crash and remove the volume (PAC failures are reported by the OS, not generated by the Go WebDAV handler), so [the opt-in integration test](go/mount/macos_mount_integration_test.go) exercises a real recursive `ditto` copy and verifies the mount remains active. `go/mount/webdav_server.go` logs non-probe handler errors without duplicating ordinary Finder 404 probes.
- `go/mount/metadata/service_write_path.go`, `metadata_write.go`, `status.go`, `worker_execute.go`, and `webdav_copy_test.go` - mount MKCOL uses `EnsureDirectoryPath` to materialize missing Desired ancestors locally when Finder sends nested directory creation out of order; it never lists an unconfirmed remote parent. Pending child mkdirs retain their current local name for pre-execution rename folding, but resolve their parent via confirmed Remote edges so a later ancestor move cannot redirect them into a provider directory that move is waiting on. `go/storage/webdav_backend.go` parses href paths once (preserving literal `+` and encoded percent sequences) rather than applying query-form decoding.
- `go/storage/sftp_backend_listing.go`, `ftp_backend.go`, and `webdav_backend.go` normalize provider mtimes to a client-`time.Local` wall-clock `ObjectInfo.LastModified`; SFTP itself only supplies an epoch and has no server-zone metadata. `go/mount/object_time.go` restores that layout with `time.ParseInLocation(..., time.Local)`, and WebDAV/Linux FUSE, Cloud Files, and WinFsp all use it rather than `time.Parse` (which silently treats it as UTC). Flutter and metadata deliberately pass the normalized string through unchanged. `object_time_test.go` and `webdav_dir_mtime_test.go` pin UTC+8 behavior. Known P2: the FTP client library still treats timezone-free legacy `LIST` dates as UTC; standard MLSD/MLST timestamps are absolute, but supporting a non-UTC legacy FTP server accurately needs an explicit server-zone setting plus an integration fixture.
- Development cache recovery: namespace bbolt state is under `~/.cloud-volume/runtime/metadata/v1/<namespace>/metadata.db`; its adjacent `namespace.json` maps the opaque namespace ID to profile/bucket. Chunk state is under the DB-persisted `schema.chunkRoot` at `metadata-chunks/<namespace>`; `CacheDirectory` determines that root only when the namespace is first created. For a fresh-development stale metadata view, rebuild or move both namespace roots only when `Status.PendingOps`, `Status.FailedOps`, and `Status.PendingContent` are all zero. Otherwise preserve the namespace rather than deleting durable intent or protected chunks.
- `lib/pages/file_manager_page_restore_sync.dart` / `file_manager_page_actions.dart` / `file_manager_object_browser_sync.dart` - page-created metadata directories register a path/profile/bucket-scoped task watch. The active listing force-refreshes when that task, or a previously active matching metadata task, reaches a terminal state, so the post-confirmation remote mtime replaces the local pending snapshot. Mounted object badges now include mkdir/write/upload/rename/copy/move/delete tasks and match both source/target paths within the active profile, preventing a same-named bucket in another account from changing the badge. Widget coverage lives in `file_manager_metadata_task_refresh_test.dart` and `file_manager_object_browser_sync_test.dart`.
...
- `go/mount/metadata/tasks.go` / `task_types.go` / `tasks_index.go` / `manager_tasks.go` - schema-v5 durable task projection uses stable `sync:<namespace>:<group>` IDs, adds sequence-qualified IDs for mixed lifecycle segments, folds pre-execution mkdir/rename chains, consecutive renames, and rapid same-inode writes, and retains raw journal events, dependency reasons, and all namespace-qualified physical snapshot IDs. `tasks_control.go` / `tasks_control_rollback.go` / `tasks_control_content.go` provide transactional cancel rollback, retry/trigger, `cancel_requested`/`reconciling` states, monotonic history compaction, and strict task-group lookup after compaction.
- `bridge/dispatch_remote_tasks.go` / `bridge/remote_task_runtime_wire.go` / `go/webapi/remote_tasks.go` / `go/webapi/remote_task_runtime_wire.go` - desktop and Web expose the same paged JSON task protocol. Metadata tasks join physical transfer monitor phases; profile/bucket/status filters, cursor pagination, freshness, and capability flags are applied before the response. Runtime projections retain local destinations, source/target paths, current-file bytes and multipart ranges. Web ignores client profile widening and uses the authenticated active profile.

#### Remote task display audit (2026-08-19, resolved in current batch)

- `go/s3/transfer_monitor_lifecycle.go` now derives kind-aware running phases, preserves source/target fields across queue-to-running transitions, keeps a queued cancellation terminal, and reference-counts nested provider phases so an outer mount task cannot finish early; legacy mount and non-S3 producers attach `ProfileID` before projection.
- `bridge/remote_task_runtime_wire.go` / `go/webapi/remote_task_runtime_wire.go` retain local destinations, source/target paths, current-file byte counters, and multipart range fields. Flutter's `RemoteTaskDisplay` renders localized phases and source-to-target summaries.
- `bridge/dispatch_app_install.go` keeps the asset name in the logical key and turns cancellation off at the installer boundary; `lib/widgets/settings_update_task_policy.dart` makes the Flutter cancel button follow that capability and conservatively disables it when the runtime snapshot is missing.
- `go/s3/transfer_monitor_current_file.go` provides seed-only resume accounting and structured part ranges, so resumed bytes no longer inflate throughput.
- `go/s3/transfer_monitor.go` / `go/s3/transfer_monitor_lifecycle.go` / `go/s3/transfer_monitor_current_file.go` / `go/s3/transfer_monitor_readers.go` / `go/sync/executor.go` / `go/storage/tracked_download.go` / `go/storage/tracked_mutation.go` - physical snapshots carry an owning profile identity, kind-aware phases, seed-only resume accounting and structured current-file/part progress. The shared mutation wrapper gives FTP, SFTP, WebDAV, and Baidu Pan copy/move/delete calls the same snapshot lifecycle as S3 without replacing an already-running mount queue. Direct page rename now sends its task ID through the tracked move path; persisted mount rename rows retain the original source plus final target. `lib/state/remote_task_store.dart` merges these snapshots with the explicit local adapter fed by `transfer_queue_remote_tasks.dart`, purges legacy `transfer:metadata-op-*` rows from older bridge versions, and reconciles complete pages instead of retaining missing remote tasks. Metadata-backed page producers may publish non-cancelable execution-only snapshots for transient upload/delete dialogs; these are excluded from `tasks` and profile aggregation. `transfer_queue.tasks.v1` is deliberately invalidated because it predates producer ownership metadata; only v2 rows are restored. All visible task rows use the unified store. The old `TransferQueue` remains an execution/local compatibility producer, never a task display source.
...
- `go/mount/metadata/service_read.go` / `go/mount/metadata/service_directory.go` / `go/mount/webdav_fs.go` / `go/mount/webdav_copy_test.go` - a Desired directory with no confirmed Remote edge is local-authoritative until its mkdir worker completes. Path resolution, explicit refresh/poll, and WebDAV's recursive Finder `MKCOL` sequence therefore do not issue SFTP `ReadDir` against an uncreated parent (which would become HTTP 409 and the misleading macOS "invalid name" dialog); protocol and service regressions cover nested creation and forced refresh.
...
- `go/mount/webdav_fs.go` / `go/mount/webdav_file.go` - Finder path: MKCOL → `createDirectory` (local + dirSync), MOVE → `webDAVFS.Rename` → `renamePath`; PUT close → staged temp renamed into hashed cache path → `stageLocalWrite`. Directory FileInfo mtime is sourced from the provider/metadata object, with a fixed epoch fallback when directory listings omit mtime, so repeated PROPFIND calls do not make Finder report a changing folder timestamp. Nested MKCOL relies on metadata's local-only Desired-directory short circuit until the remote marker is confirmed.
...
- Remote-operation UI is unified: `lib/models/remote_task.dart`, `remote_task_display.dart`, `lib/state/remote_task_store.dart`, `lib/pages/transfers_page*.dart`, `lib/widgets/remote_task_widgets.dart`, `lib/widgets/sidebar_transfer_status.dart`, and `lib/pages/file_sync_tasks_page.dart` render effective operations, dependencies, expandable raw events, physical phases, cancel/retry/trigger, and history. Mount range reads render their file path as the primary label and their current `bytes=` interval as `读取范围`; no page/sidebar/sync card falls back to `TransferQueue`. History cleanup is explicit: selecting terminal rows sends their IDs through `clear_remote_task_history`, and Go `ClearTaskHistoryIDsFor` removes only those terminal journal entries (retaining anything that still protects later dependencies); the unselected "清理历史" action removes all compactable terminal history in scope. Metadata physical snapshots (`metadata-op-<namespace>-<seq>`) are never projected as local rows — they are already embedded in the durable task.
- Task-row visuals (2026-08-28, title updated 2026-08-29): `RemoteTaskRow` in `lib/widgets/remote_task_widgets.dart` lays out [checkbox 18 + gap 10 + `_KindIconChip` 28 + gap 12 | title/subtitle | right controls]; the title column therefore starts at **80px**, and the shared `remoteTaskContentIndent` const (80, defined in `remote_task_style_helpers.dart`) is the binding indent for the expanded `RemoteTaskDetails` panel and the row divider — keep all three aligned when changing leading chrome. The title is **entry name only** (`remoteTaskEntryName` in `remote_task_style_helpers.dart`: last non-empty path segment of `operationPath`, kind-label fallback) — the kind chip carries the op type, and `RemoteTaskDetails` opens with 操作/所属桶/完整路径 lines so the verb, bucket, and full path never return to the row title. Since 2026-08-29 the expanded panel lives in `lib/widgets/remote_task_details.dart` (`RemoteTaskDetails`, public): every row is a fixed 60px label column (`_detailLabelWidth`) + wrapping value so metadata, events, and transfer IDs align; journal events render under one 事件记录 group label (blank labels for subsequent rows, same pattern as 传输) with `#<seq> remoteTaskEventKindLabel(path)（已合并）` as the value; physical task IDs print one per line (first carries the 传输 label) instead of a `·`-joined blob. Active states (running/verifying/cancel_requested/reconciling) show one compact 12px spinner next to the `RemoteTaskStatusBadge` pill (tinted fill + hairline border); action buttons sit right of the badge. `RemoteTaskDetails` uses a 3px kind-colored accent bar (`IntrinsicHeight` row; uniform-border rule forbids a left-only BorderSide with radius) over a muted panel of label-column rows (依赖 amber, 错误 destructive). Pure lookups (`remoteTaskSpeedSummary`, `remoteTaskSubtitle`, `remoteTaskEntryName`, `remoteTaskStatusLabel/Color`, `remoteTaskKindIcon/Color/Label`) live in `lib/widgets/remote_task_style_helpers.dart` — import it directly (transfers_page.dart, sidebar_transfer_status.dart do); `remote_task_widgets.dart` no longer defines them, so the file stays under the 500-line cap. `RemoteTaskStatusBadge`'s public API is shared with `batch_task_progress_dialog.dart`. Loading (2026-08-28, same batch): the queue's first history read renders `_RemoteInitialLoading` (standard 22/2.4 body spinner + 「正在加载任务…」) inside the list card when `store.tasks.isEmpty && store.isLoadingInitialHistory`; since 2026-08-29 the in-list pager row below the rows replaces the former fixed footer, and all queue spinners (batch buttons, pager busy row, row status/acting) are `AppLoadingIndicator` per the loading binding above. **Known P2/P3 (review 2026-08-28):** P1 fixed same day — `_detailLine` must keep unbounded wrapping (it is the only full-text surface for long provider errors / dependency reasons; the row subtitle intentionally stays 1-line + ellipsis). P3: `remoteTaskContentIndent = 80` duplicates leading-chrome arithmetic (12+18+10+28+12) guarded by its helper doc comment — both consumers import the one shared const since the review follow-up; label texts are single-line ellipsis-guarded, `remoteTaskEventKindLabel('unknown')` → 未知操作, `test/remote_task_details_test.dart` pins the label-column layout, per-line transfer IDs, and the unbounded value-wrap contract (review 2026-08-29 P3: the 原始操作 rawEventCount fallback branch is untested; the （已合并） suffix is conditional on `event.folded`, docs' shorthand notwithstanding). `Colors.amber.shade700` for 依赖 is light-palette-only, revisit with dark mode; the `remote_task_widgets.dart` cap-relief split landed 2026-08-29 (`RemoteTaskDetails` in its own file, widgets file ~327 lines; `_row`'s value column inherits the unbounded-wrap rule the old `_detailLine` pinned); `_RemoteInitialLoading` shows only when the history tab/all filter is active — under 进行中/等待中/失败 filters a first activation renders 「暂无任务」 first (pre-existing early-return, deferred); `transfers_page_remote.dart` sits near the 500-line cap (review 2026-08-29) — split queue tabs/filters out before the next addition. Phase display (2026-08-29): metadata tasks' Go `Phase` is a subsystem marker (`provider`/`verification`/`dependency`/`history`, set in `go/mount/metadata/tasks.go`), not a user phase — `RemoteTaskDisplay.phaseLabel` maps those to `''` and also suppresses any `phaseDetail` starting with `cancel requested` (the cancel/reconcile English sentences) so terminal rows show 已完成/已取消 and cancel rows 正在取消/正在对账 from badge/subtitle instead of leaking raw English; `BlockedReason == 'waiting for journal dependencies'` localizes to 等待前置操作完成 via `remoteTaskBlockedReasonLabel` (used by subtitle + 依赖 detail line), the sidebar active-row fallback is `remoteTaskStatusLabel` (not a hardcoded 进行中), and real physical phases (uploading/quiet_period/mount_read/…) keep their labels. `test/remote_task_display_test.dart` pins all of these. Entry-name titles (review 2026-08-29): same-name rows in one view are disambiguated only via details (consider parent-dir in subtitle later); `RemoteTask.name` and `remoteTaskEntryName` are two near-identical derivations with different precedence — consolidate before adding a third; rename tasks show only their new path in details (extend `sourceTargetSummary` to rename or drop the duplicate 完整路径 when the summary exists); sidebar hover rows and sync-card one-liners intentionally keep full paths (no expand panel there), and `_KindIconChip` carries the op verb as an `AppTooltip`.
...
**Known P2/P3 (review 2026-08-29, PR #5 merge):**
- P3 — `remote_task_store_polling.dart` Android history auto-reload fires `loadInitialHistory()` from the store-global poll loop even while the TransfersPage `IndexedStack` child is inactive (bandwidth churn only; the serialized read gate and strictly-increasing `historyGrew` guard keep correctness). Deferred.
- P3 — combined diff additionally contains `config_setup_page.dart` + `transfers_page.dart` trivial re-applications of main's `AppLoadingIndicator`/import merge over the PR's older copies; no behavior change, nothing to fix.
...
- `go/mount/webdav_logging.go` / `webdav_request_context.go` / `webdav_fs.go` / `webdav_lock_null.go` - x/net/webdav's missing-resource `LOCK` path uses the same `O_RDWR|O_CREATE|O_TRUNC` flags as `PUT`; the logging wrapper carries the HTTP method through the request context so a missing `LOCK` returns an in-memory lock-null file instead of staging a zero-byte metadata write. A later `PUT` still takes the normal writable path, including an empty payload. `webdav_lock_null_test.go` pins both cases.
- `go/mount/metadata/service_write_path.go` / `metadata/write_path_fast.go` - Profile-scoped path writes stage protected content-addressed chunks first, then commit the inode, generation, chunk nlinks, `ContentRef`, and journal indexes in one bbolt transaction. This replaces the old ensure-inode/reserve-generation/ref/write sequence while retaining exact-generation worker retirement and crash-sweep cleanup. The opt-in real `mount_webdav` + `ditto` benchmark copies 117 files (3,565,158 bytes): pre-fix it took about 30.1s with 234 metadata writes (117 zero-byte `LOCK` probes); after lock-null handling and Finder temp-overlay handling it took 6.93s with 117 metadata writes and no zero-byte writes. The remaining foreground time is local APFS durability work for each final MOVE plus macOS WebDAV protocol traffic, not remote upload, which stays behind the quiet barrier. A conservative namespace protection manifest is fsynced once at the start of a burst and finalized after 250ms idle; valid manifests and active splice files remain protected from cache cleanup, and a service close prevents the deferred finalizer from touching a closed bbolt handle.
- `go/mount/webdav_finder_temp.go` / `local_overlay.go` / `overlay_bridge.go` / `webdav_finder_temp_test.go` - Finder's per-file `.BC.T_*` copy staging files are a local overlay path. Their `PUT` never enters the Desired tree; their final cross-boundary `MOVE` copies bytes into the normal cache path and creates one final-name metadata write. AppleDouble `._.BC.T_*` to `._*` moves remain fully local overlay metadata. This is intentionally separate from general dot-file handling so user-visible dot files still sync normally.
...
- `go/mount/macos_mount.go` / `macos_mount_stop.go` / `macos_mount_test.go` / `macos_mount_integration_test.go` / `macos_command.go` / `macos_command_test.go` / `system_mounts.go` - Default mounts invoke non-interactive `mount_webdav -S -v <mountName>`, then wait up to 30s for the exact random loopback URL plus mount path to appear in `mount -t webdav`. Command success alone is not registration, but a command error or timeout is never recovered into a successful mount from a table row. `/var` and `/private/var` aliases compare through the resolved parent path without touching the mounted root. Failed attempts clean up for up to 45s; `mountAttempted` keeps server/access ownership retryable if system unmount fails, and active-state polling stays URL-qualified until that attempt is confirmed so a foreign same-path volume cannot be adopted. Finder `open` is asynchronous and guarded by a per-path single-flight gate because a process blocked in a WebDAV filesystem call may ignore `CommandContext` cancellation for much longer than five seconds. The opt-in real lifecycle test runs with `CLOUD_VOLUME_RUN_MACOS_MOUNT_INTEGRATION=1`; normal tests mock the external command.
...
- `go/mount/macos_mount.go` `findMountedWebDAVPath`/`probeMountedWebDAVPath`/`waitForMountedWebDAVPath` require the row's source URL to equal `serverURL` after `normalizeServerURL` (lowercased scheme/host, trailing slash on path). A row with the same display name but a different port — a stale volume, a different process's mount, or the directory created by the requested-path branch's `MkdirAll` — is rejected. `canonicalMountPath` resolves only the parent so `/var/...` and macOS's reported `/private/var/...` match without invoking a potentially blocking operation on a live WebDAV root.
...
- Automatic WebDAV prewarming was removed (2026-08-18): `os.Stat`/`os.ReadDir` have no context cancellation and a wedged `webdavfs_agent` syscall can keep the mount busy after a timeout. Finder's first access now performs the normal initialization, while mount/unmount lifecycle avoids spawning an un-reapable filesystem syscall.
...
- `/sbin/mount_webdav -S` runs without hidden authentication/non-responsive-server UI. Its successful process exit means the mount request was accepted, not necessarily that the system has published the volume; startup therefore waits for the exact URL/path row after success. Conversely, a process error or timeout fails closed and may only trigger cleanup, never a successful recovery from a mount-table row. Do not reintroduce early-return/cancel behavior or use `statfs`/`os.Stat`/`os.ReadDir` as the completion probe: Apple's first VFS access can block independently of registration.
- **Default path permissions (2026-08-18):** `/Volumes` is root-owned on normal macOS installs. `prepareMacOSMountPath` falls back only for an unwritable default managed path `/Volumes/云卷-<bucket>`, creating `~/云卷/云卷-<bucket>` and returning that actual path in mount status. Explicit custom paths still fail rather than being silently relocated. Stale-mount cleanup scans both managed roots.
...
- `go/storage/sftp_backend*.go` - Implements SFTP listing, file I/O, and mutations through `pkg/sftp`; `sftp_backend_quota.go` uses the optional `statvfs@openssh.com` extension when offered by the server. `HeadObject` maps only a real missing path to `os.ErrNotExist` and preserves permission/connection failures so metadata verification can retry with an actionable error.
