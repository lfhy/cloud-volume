...
- `go/mount/metadata/types.go` - durable contracts: `Inode`, `Dirent`, `Op`, `ListingState`, `ContentRef`, `Object/Page/Cursor`, `Namespace`, and the narrow provider `Backend` interface. Inode identity is local to each namespace; no symlinks/hardlinks.
- `go/mount/metadata/store.go` / `store_io.go` - bbolt schema v1 with `schema/inodes/dirents/journal/listing_state/content_refs/ready_ops/inode_ops`. Root inode is fixed at 1; inodes are monotonically allocated and never reused. Every write is serialized; schema mismatch returns an error (reset/rebuild, no upgrade).
