...
- `go/mount/bucket_access_writes.go` / `bucket_cache_rename.go` - `createDirectory` only stages locally + enqueues marker. `stageLocalWrite` protects the marker-to-queue handoff. Synchronous `renamePath` (macOS WebDAV, Linux FUSE, WinFsp) holds that gate, rebases/fences directory markers, drains matching queued/running uploads, then moves the remote source; cache byte moves are preflighted and indexed only after success, with best-effort rollback on a multi-file failure. `enqueueRenamePath` (Windows Cloud Files only) captures a scoped persisted `mutationRecord` with barrier generation and a legacy `run` closure that creates the renamed directory remotely when the old path is absent.
