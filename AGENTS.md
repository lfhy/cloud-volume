...
- `RemoteTaskStore.canLoadMoreHistory` exposes the first history request even though active-only polling has no history cursor. `TransfersPage` must render that affordance before its empty/filter-empty states, otherwise a history-only queue is unreachable. A cached row is removable history only for `done`/`canceled`; `failed`/`conflict` remain in the active-only response and retry queue until they disappear remotely. Desktop and Web queue classifiers normalize runtime `pending` as waiting and `completed`/`cancelled` as history to match Dart wire parsing.
