...
- `go/mount/manager.go`, `system_mounts.go`, and `macos_mount_stop.go` - macOS status polling probes `mount -t webdav`; a transient probe error now keeps the live session and records a diagnostic, while a confirmed missing mount records an unexpected-disconnect error before releasing the local server/access. macOS `webdavfs_agent` can independently crash and remove the volume (PAC failures are reported by the OS, not generated by the Go WebDAV handler), so [the opt-in integration test](go/mount/macos_mount_integration_test.go) exercises a real recursive `ditto` copy and verifies the mount remains active. `go/mount/webdav_server.go` logs non-probe handler errors without duplicating ordinary Finder 404 probes.
- `go/mount/metadata/service_write_path.go`, `metadata_write.go`, and `webdav_copy_test.go` - mount MKCOL uses `EnsureDirectoryPath` to materialize missing Desired ancestors locally when Finder sends nested directory creation out of order; it never lists an unconfirmed remote parent. `go/storage/webdav_backend.go` parses href paths once (preserving literal `+` and encoded percent sequences) rather than applying query-form decoding.
