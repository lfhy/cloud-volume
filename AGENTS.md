...
- `go/mount/metadata/worker.go` / `worker_execute.go` / `worker_move.go` / `status.go` - scheduler/executor split for quiet-period journaling, retry backoff (15s→2m), pre-upload fingerprint checks, confirmation, conflict marking, and transfer snapshot IDs `metadata-op-<seq>`. A write executes against its immutable journal parent/name Remote edge, then a later rename moves it; move source/target and confirmation parent/name are frozen before the provider call, so a later Desired rename cannot redirect replay. A generic provider MOVE error is reconciled by probing both frozen edges; only a matching target fingerprint (or matching explicit directory marker) can establish a pre-persist side effect. Directory moves/deletes wait for earlier descendant work. `claimDue` also waits on same-inode and prior parent work. `Drain` scans journal running entries absent from `ready_ops`; deletes purge confirmed subtrees. `remoteTarget` falls back to Desired only for never-synced deletes.
...
- `go/s3/transfer_monitor.go` / `bridge/dispatch.go` / `lib/state/transfer_queue.dart` - the visible Flutter transfer queue currently projects runtime transfer snapshots, not the full metadata journal. `OpMkdir` has no transfer snapshot; `OpRename`/`OpDelete` only appear when a provider implementation consumes the task ID (S3 move/delete paths do, FTP/SFTP/WebDAV/Baidu commonly ignore it); `OpWrite` appears as `metadata-op-<seq>` only when the worker starts the upload, not while it is waiting on quiet period or dependencies. Therefore a local mkdir-to-rename-to-write sequence is durable and ordered in bbolt but is not yet shown as a truthful pending-operation list. If mkdir and rename both happen before the worker runs, the worker resolves the current Desired name and can create only the final directory, making the rename a no-op confirmation; if mkdir already reached the provider, a real move follows. A future journal-backed task projection must expose blocked reasons, coalesced effective operations, retries, and namespace-qualified IDs rather than treating transfer snapshots as the source of truth.
