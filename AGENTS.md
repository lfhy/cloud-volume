...
- `go/mount/metadata/worker.go` / `worker_execute.go` / `status.go` - scheduler/executor split for quiet-period journaling, retry backoff (15s→2m), remote source/target derived from Remote/Desired inode edges, pre-upload fingerprint checks, HEAD confirmation, conflict marking, and transfer snapshot IDs `metadata-op-<seq>`. A write executes against its immutable journal parent/name Remote edge, then a later rename moves it; confirmation leaves the inode pending while a later same-inode op exists, so refresh cannot revive the old Remote key. `service_read.go` also refuses a same-name listing entry in an unmaterialized rename target unless it matches the inode's confirmed Remote edge, so the later move cannot collapse to a no-op. Directory moves/deletes wait for earlier descendant work. `claimDue` also waits on same-inode and prior parent work. `Drain` scans journal running entries absent from `ready_ops`; deletes purge confirmed subtrees. `remoteTarget` falls back to Desired only for never-synced deletes.
...
- `go/mount/metadata/worker_execute.go` persists `Op.MoveApplied` after a provider accepts a move and before HEAD confirmation. Replay only re-confirms this phase; the rare pre-persist crash window reconciles a missing source only when the target fingerprint proves the move. `worker_ordering_test.go` covers the reopen/materialize/retry sequence.
- **Known P2/P3 (M6c review scope):** page task IDs do not yet map to worker transfer snapshot IDs, mount byte reads cannot yet serve unconfirmed page-upload chunks, and Web API/browser upload endpoints remain provider-direct even with `ProfileID`. Also reject `.`/`..` page create/rename segments in a later bridge-input validation pass. Do not present any of these as completed durability behavior.
