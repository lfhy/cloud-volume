...
- `go/mount/metadata/status.go` / `worker_move.go` - confirmation persists a non-empty provider `HeadObject.LastModified` into `Inode.MTime` alongside the remote edge and fingerprint, except while a later unsettled write owns a newer local timestamp. This keeps a newly created SFTP directory's timestamp visible immediately after sync without letting an older confirmation overwrite a rapid later write; `worker_mtime_test.go` pins both cases and `sftp_backend_test.go` pins the actual SFTP `Stat` source. Directory materialization remains the listing-path source of remote timestamps, and the Dart browser intentionally leaves only genuinely empty values blank.
