...
- `go/mount/metadata/store.go` / `store_io.go` - bbolt schema v4 with `schema/inodes/dirents/journal/listing_state/content_refs/chunks/ready_ops/inode_ops/task_groups/task_members`. Root inode is fixed at 1; inodes are monotonically allocated and never reused. Every write is serialized; schema mismatch returns an error (reset/rebuild, no upgrade).
...
- `go/mount/metadata/tasks.go` / `tasks_index.go` / `manager_tasks.go` - schema-v4 durable task projection uses stable `sync:<namespace>:<group>` IDs, folds pre-execution mkdir/rename chains, consecutive renames, and rapid same-inode writes, and retains raw sequences, dependency reasons, and all physical snapshot IDs. `tasks_control.go` / `tasks_control_content.go` provide transactional cancel rollback, retry/trigger, `cancel_requested`/`reconciling` states, and scoped 30-day history compaction.
...
- `go/s3/transfer_monitor.go` / `go/sync/executor.go` - physical snapshots carry an owning profile identity so sync cards can use `RemoteTaskStore.tasksForProfile()` without parsing `sync-...` IDs. The old `TransferQueue` remains an execution/local compatibility producer, never a task display source.
