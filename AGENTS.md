...
- `go/mount/metadata/manager.go` - namespace registry (see above) plus `DrainAll`, which drains all namespace workers concurrently so app-exit drain time is one slowest-worker timeout, not a sum.
