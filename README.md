...
- 文件管理：桌面端聚合 bucket / 根目录，支持目录浏览、列表/网格视图、搜索、多选、右键菜单、拖拽上传、粘贴上传、复制、移动、重命名、新建目录、下载和批量删除；Android 使用独立的触控文件页，以大搜索框、单列大行、行尾操作和 FAB 组织同一套文件操作，不提供列表/图标模式切换。
...
Android runner 已包含在仓库中，并把 ARM64 Go FFI bridge 作为 `libremote_storage_bridge.so` 打进 APK。移动端可使用账号配置、对象浏览、上传下载、分享及回收站等远程存储功能；移动端会禁用桌面专属的本地挂载、目录同步、多窗口、外部应用打开、缓存目录打开、跨应用拖放和文件 URI 剪贴板操作。Android 首屏就是独立维护的文件管理页：操作收进顶部 `…` 和 FAB 的动作面板，搜索固定在内容顶部，桶行沿用既有的 LocalCloudPan 蓝色桶图标，不提供列表/图标模式切换。底部导航栏可在 设置 → 常规 → 底部导航 自定义（2–5 项；默认 文件·账号·任务·回收站·设置，设置必须保留）；旧版本首页配置会自动迁移为设置入口。进入桶、目录或桶回收站后，左上返回和安卓返回键都会先回到刚才的文件位置，再按访问历史回退页面，历史耗尽才退出应用。
