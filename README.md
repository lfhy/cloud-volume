...
- 文件管理：桌面和 Android 共享 bucket / 对象 / 回收站数据、请求、mutation 与返回栈，但各自维护呈现层。桌面保留面包屑、网格/列表切换、挂载和右键菜单；Android 使用「文件 + 当前位置」页头、常驻搜索、固定紧凑列表与行尾 `…` 底部抽屉，桶、文件和目录均在抽屉中按需显示动作，桶设置不再占用行内入口，也不暴露挂载入口。对象可长按打开同一抽屉，多选保留 48dp 选择区；当前目录只读时会隐藏写入动作。进入桶、目录或桶回收站后，顶栏返回按钮（包括加载和错误状态）与安卓系统返回共用同一位置栈。
- 预览与缓存：桌面端支持图片预览、外部应用打开、另存为和下载；Android 支持图片与 Markdown 内嵌预览、下载到应用缓存和经系统应用选择器打开，不提供系统另存为。已下载文件会复用本地缓存，本地拖拽或粘贴上传成功后也会把本地副本登记为预览缓存，刚上传的文件双击即可打开。
...
Android runner 已包含在仓库中，并把 ARM64 Go FFI bridge 作为 `libremote_storage_bridge.so` 打进 APK。移动端可使用账号配置、对象浏览、上传下载、分享、回收站、Markdown 内嵌预览、下载到应用缓存与系统应用打开等远程存储功能；预览不提供系统另存为。移动端会禁用桌面专属的本地挂载、目录同步、多窗口、缓存目录打开、跨应用拖放和文件 URI 剪贴板操作。Android 文件页独立维护移动 chrome：顶部显示「文件」和当前位置、搜索框常驻、桶行尾 `…` 用底部抽屉展示打开/桶设置/回收站/WebDAV 等适用动作；桌面菜单与挂载入口不会出现。系统状态栏在浅色应用背景下使用深色图标。底部导航栏可在 设置 → 常规 → 底部导航 自定义（2–5 项；默认 文件·账号·任务·回收站·设置，设置必须保留）；旧版本首页配置会自动迁移为设置入口。进入桶、目录或桶回收站后，安卓返回键会先回到刚才的文件位置，再按访问历史回退页面，历史耗尽才退出应用。
