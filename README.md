...
- 文件管理：桌面与 Android 共享同一套文件管理 UX 和组件树，统一提供 bucket / 根目录浏览、面包屑、搜索、多选、操作栏、复制、移动、重命名、新建目录、下载和批量删除。桌面支持列表/网格切换与挂载操作；Android 沿用同一信息架构但固定列表模式并隐藏挂载/卸载/打开挂载目录入口，以长按和行尾操作按钮提供同一组对象命令。进入桶、目录或桶回收站后，顶栏显示返回按钮，加载或错误状态也可返回上一文件位置。
