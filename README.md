...
- 文件管理：桌面和 Android 共享 bucket / 对象 / 回收站数据、请求、mutation 与返回栈，但各自维护呈现层。桌面保留面包屑、网格/列表切换、挂载和右键菜单；Android 使用「文件 + 当前位置」页头、常驻搜索、固定列表与行尾 `…` 底部抽屉，桶设置只在抽屉中显示，不暴露挂载入口。进入桶、目录或桶回收站后，顶栏返回按钮（包括加载和错误状态）与安卓系统返回共用同一位置栈。
