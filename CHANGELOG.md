...
- 修复任务队列历史总数与已显示行数不一致却找不到入口：少于 100 条历史会完整显示，更多历史使用明确分页；列表卡片固定底部显示「历史已显示 N / 总数」和「加载下一页（还剩 N 条）」；WebSocket 继续即时刷新活跃任务与总数，每次历史总数增长都会作废旧 offset cursor、从首页重新打开分页，避免连续完成任务时跳过最新行，也不再把继续加载按钮埋到滚动列表末尾。
...
- 任务队列首次切入现在会自动加载最多 100 条历史；小队列不会再要求先点“加载更多历史”，大队列首次加载会等待并复用 active-only 快照，期间在固定分页条显示 loading。清理历史后会使旧分页 cursor 失效，避免删除后又从过期位置加载出错位的下一批记录。
