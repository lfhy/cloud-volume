...
- Android 文件管理重构为独立维护的触控页面：文件直接作为首屏，采用大搜索框、单列大触控行、行尾 `…`、FAB 与操作抽屉；不再提供移动端首页或列表/图标模式切换，桌面文件管理保持原有页面。两端共享文件加载与操作工作区，不共享页面布局。底部导航默认调整为 文件·账号·任务·回收站·设置，旧首页配置自动迁移为设置入口；安卓返回键先处理目录和桶回收站，再回退 tab 历史，最后退出应用。
- 修复 Android 文件页的桶图标与返回链路：桶行继续使用既有 LocalCloudPan 蓝色桶图标；进入桶、目录或桶回收站时立即记录文件位置，加载中或加载失败仍显示左上返回。系统返回和左上返回会优先回到刚才的文件位置，迟到的旧请求不能把页面重新带回已离开的目录；从子目录打开桶回收站后返回也会恢复该子目录。
