...
- Android 文件管理改为以桌面端为唯一 UX：标题、面包屑、搜索、操作栏、列表/网格切换和 bucket / 对象 / 回收站浏览器共用同一组件树，删除独立触控列表、FAB 和动作抽屉。Android 只保留安全区、文件位置返回栈及长按/行尾 `…` 触摸入口；进入桶、目录或桶回收站后，系统返回优先回到刚才的文件位置，迟到请求不能把页面重新带回已离开的目录。
- Android 文件管理沿用桌面端信息架构但固定为列表模式，不显示网格/列表切换按钮，也隐藏挂载、卸载和打开挂载目录入口；进入桶、目录或桶回收站后，顶栏始终提供返回按钮，包括加载和错误状态。
