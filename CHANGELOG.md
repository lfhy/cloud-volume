...
- Android 应用内拟态框统一改为全屏 surface，不再在状态栏和导航栏之间露出上下暗色遮罩；内容继续避让安全区与软键盘，系统栏图标随明暗主题切换，系统返回/关闭按钮/结果回传语义不变。桌面与 Web 仍保持有限宽居中弹窗。远端目录选择器、同步编辑器和文件预览使用 fill-height 主体，横屏 + 软键盘的低高度场景可整体滚动，窄屏连接字段、深层面包屑及所有表单 footer 按钮可分行，避免全屏后内容继续溢出。
