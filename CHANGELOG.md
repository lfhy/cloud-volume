...
- Fixed journal replay after a remote rename succeeds but target confirmation is interrupted: the durable move phase now confirms rather than retries a missing source. Page-to-mount cache projection now carries an inode/revision guard, so a delayed page callback cannot hide a newer mount write.
