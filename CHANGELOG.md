...
- 修复任务页切换时的 `setState() or markNeedsBuild() called during build`：历史自动加载现在在帧结束后启动，侧边栏传输动画也会延后同步，避免 `ScaleTransition` 在父树构建中被标脏。任务页新增始终按后端历史总数显示的「清理全部历史」按钮，可不选中当前 100 条就清理全部可清理历史。
- 任务队列首次切入现在会自动加载首屏（最多 100 条）历史记录，不再要求先点“加载更多历史”；首次加载会等待并复用 active-only 快照，期间只显示小型 loading。清理历史后会使旧分页 cursor 失效，避免删除后又从过期位置加载出错位的下一批记录。
