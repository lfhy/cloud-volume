...
- Android 文件管理改为独立移动端呈现层：使用「文件 + 当前位置」页头、常驻搜索和固定列表，桶的行内设置图标移除，行尾 `…` 以底部抽屉提供打开存储桶、桶设置、回收站与 WebDAV 等适用动作；挂载相关入口继续完全隐藏。桌面仍保留原有面包屑、操作栏、网格与右键菜单，两个平台共用数据加载、mutation 和文件位置返回栈。应用根明确设置 Android 浅色系统栏样式，恢复状态栏时间、信号和电量的深色图标显示。
