...
- Android 应用内拟态框统一改为全屏 surface，不再在状态栏和导航栏之间露出上下暗色遮罩；内容继续避让安全区与软键盘，系统栏图标随明暗主题切换，系统返回/关闭按钮/结果回传语义不变。桌面与 Web 仍保持有限宽居中弹窗。远端目录选择器、同步编辑器和文件预览使用 fill-height 主体，横屏 + 软键盘的低高度场景可整体滚动，窄屏连接字段、深层面包屑及所有表单 footer 按钮可分行，避免全屏后内容继续溢出。滚动内容另预留 Shad 输入框的向外焦点环绘制边距，不再因为贴住 viewport 而被裁掉左右边。
- 修复 Android 首启「从备份存储还原」目录弹窗:手机宽度下四个底部按钮会按组换行,目录行也不再被桌面固定元数据列挤成省略号;WebDAV、百度网盘、FTP、SFTP 的唯一合成桶显示为「备份存储」并直接进入,真实桶 identity 保持不变,S3 继续显示真实桶列表。空或失效的初始 bucket 不再把 `cloud-volume-config-backups` 强套到首个存储并触发 `file does not exist`;返回有效目录会清除旧错误,远端快照为空时显示明确空状态而不永久转圈。
