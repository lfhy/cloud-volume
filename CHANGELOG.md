...
- 修复任务详情「阶段」显示原始英文（history/provider/verification/dependency）：metadata 任务的 Phase 表达的是归属子系统而非用户阶段，现已在展示层过滤；终态任务不再显示「阶段： history」，副标题与徽标回落为「已完成/已取消」等可读状态，真实物理阶段（上传中、静默期、挂载读取等）不受影响。
