...
| 域 | 职责摘要 |
|---|---|
| [account_management](features/account_management.md) | 账号管理页/三步向导、账号禁用、首启配置、桶可见性与别名、每桶配额与远端配额发现、自定义排序、多账号桶加载韧性(并发/超时/singleflight/负缓存/3s 拨号超时)。 |
...
| 域 | 职责摘要 |
|---|---|
| [app_shell](features/app_shell.md) | 窗口关闭/托盘退出确认、macOS 窗口生命周期与常量、侧栏导航结构与安卓底栏(纯主题背景 + 强调色选中)、应用图标生成链、响应式页面头部操作区。 |
