...
- `lib/widgets/file_manager_object_browser.dart` / `file_manager_object_browser_helpers.dart` / `file_manager_object_browser_mobile.dart` — 桌面对象浏览继续使用表头、卡片、网格和 `DesktopContextMenuRegion`;Android 固定为 `FileListTile(compact: true)` 行，隐藏这些桌面 chrome 与本地挂载同步徽标。对象行使用 28dp 图标、48dp 选择命中区和含对象名语义的 48dp `…`;行尾或长按都经 `showAppModal` 打开动作抽屉。已选多项时抽屉改以「已选 N 项」为标题并提供批量动作；桌面共享的行 helper 保持对象打开、选择和删除态逻辑一致。桶级动作由 `file_manager_bucket_browser_mobile.dart` 进入 Android 底部抽屉，桌面继续使用上下文菜单。
