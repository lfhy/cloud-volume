...
- `lib/pages/mobile_file_manager_page.dart` / `mobile_file_manager_presentation.dart` — Android 无状态入口把 `mobileNavigation` 与 `MobileFileManagerNavigation` 交给 `FileManagerWorkspace`；独立呈现层渲染与桌面导航同名的「文件管理 + 桌面同款首页说明/进入后的当前位置」页头、常驻搜索和右上 48dp `+` 操作入口。桶/目录位置的入口经 `showAppModal` 底部抽屉列出新建目录和上传；全局回收站经 `SidebarItem.trash` 独立导航进入。桶回收站位置列出返回文件和可用时的清空操作，操作不可用时入口隐藏。二级位置的 48dp 顶栏 Back 与系统 Back 共用位置栈，且在加载、错误状态也可用。
