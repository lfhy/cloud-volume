...
- `lib/widgets/mobile_file_manager_surface.dart` / `mobile_file_manager_browser.dart` — Android chrome 与单列浏览器:状态栏 `SafeArea`、48px 搜索、大触控行、行尾 `…`、无列表/网格切换。桶行继续使用既有 LocalCloudPan `sidebar/bucket.svg`，对象行同样保留 `LocalCloudPanFileIcon`。可点击行 idle 使用 `basic` cursor,hover 才切 `click`(见 [ui_rules](ui_rules.md))。
