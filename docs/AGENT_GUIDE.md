...
5. 开始新的调查前,先查 [PROJECT_GUIDE.md](PROJECT_GUIDE.md) 是否已有结论;改动涉及既有设计决策时,先查 [notes/](notes/README.md)(决策记录)里是否已有持有该决策的 Agent Note。不要重复已记录过的探索。
...
- **决策记录(binding):** 每个非平凡变更(行为、架构、跨文件契约、流程/工具、测试策略、磁盘/wire/配置格式)在同一变更集中新增或更新至少一条 Agent Note——记录为什么、放弃了什么替代方案、后果与验证;更新已持有该决策的现有笔记即满足,不要建重复。格式与目录规则见 [notes/README.md](notes/README.md)。纯机械/局部改动豁免。
