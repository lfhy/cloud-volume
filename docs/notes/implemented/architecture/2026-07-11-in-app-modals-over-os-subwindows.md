...
- 业务 surface 统一构造 `AppShadDialog`:Android 使用键盘后可用约束全屏、背景延伸到系统栏且内容留在 SafeArea,桌面/Web 仍透传有限宽 `ShadDialog` 行为。系统栏样式由 route 内 `AnnotatedRegion` 管理,不写泄漏到下一页的命令式 `SystemChrome` 状态;可滚动内容在 viewport 内默认预留 6px,完整容纳 Shad 向外绘制 4px 的输入焦点环。
...
- Android 滚动内容的可用宽度默认减少 12px;这是保留完整焦点可见性的代价,窄屏 footer 由 `Wrap`/`OverflowBar` 继续承接提前换行。
