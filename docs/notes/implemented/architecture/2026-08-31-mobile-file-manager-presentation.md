...
- 页面级文件动作只由右上 `+` 打开；普通桶/目录只提供新建目录与上传，全局回收站保持独立导航入口，桶回收站管理项仍按状态生成，按钮与每一抽屉行均保持 48dp。
...
`test/app_tooltip_test.dart` 锁定 Android 只得到语义标签、没有 `ShadTooltip`，桌面仍保留可视提示；`test/widget_test.dart` 断言 Android 不出现桌面 `FileManagerActionBar` 或 `FileManagerBreadcrumbBar`、使用常驻搜索与独立页头、桶行没有行内桶设置图标，二级页的右上 `+` 没有触摸 tooltip、保留「文件操作」语义并以 48dp 抽屉行显示新建目录和上传、没有「回收站」或「打开回收站」；独立底栏入口仍可显示和清空回收站，桶行 `…` 仍保留「打开回收站」。二级页返回按钮没有触摸 tooltip，`…` 底部抽屉保留桶设置且没有挂载项；打开抽屉后经系统 Back 关闭时，不得残留操作表面。`test/file_manager_object_browser_mobile_test.dart` 锁定对象紧凑行、48dp 行尾和选择命中区、长按/系统 Back、只读动作过滤、多选目录下载门控及窄屏大字号抽屉。它们同时锁定列表模式、已加载/加载中二级页的顶栏 Back、文件 Back、同步跳转、分页和 mutation 的迟到响应。`flutter analyze` 与完整 Go / Flutter 测试集验证呈现分离不改变共享运行时。
