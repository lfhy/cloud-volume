...
平台差异只留给输入与系统边界：桌面/Web 才包裹原生拖放和 file URI 剪贴板；Android 加安全区和紧凑 padding，以长按或紧凑对象行尾 `…` 打开同一份上下文菜单。列表/网格切换和操作栏保持可见，不能再用 Android 专用操作抽屉替换。
...
- 文件管理 UX 的每次视觉或交互变更默认同时覆盖桌面和 Android。
...
`test/widget_test.dart` 同时断言 Android 出现 `FileManagerActionBar`、`FileManagerBreadcrumbBar` 和共享 bucket / 对象 / 回收站浏览器，并覆盖 Android 文件 Back、同步跳转、分页和 mutation 的迟到响应。`flutter analyze` 与完整 Go / Flutter 测试集验证共享呈现和既有文件操作。
