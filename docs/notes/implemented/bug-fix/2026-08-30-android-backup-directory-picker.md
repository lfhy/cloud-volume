...
同一选择器的桶、目录、文件行复用了桌面 `FileListTile` 的固定元数据列,在手机上会把 `boot`、`home` 等短名称也挤成省略号。WebDAV、百度网盘、FTP、SFTP 的唯一合成桶还会暴露登录用户名等后端名字(常见为 `root`),但配置备份需要稳定的「备份存储」语义,同时不能改写桥接和持久化使用的真实桶 identity。设置页的已保存位置预览也不能在打开选择器前泄漏同一个原始名字。失败后旧错误不会随成功导航清除,空快照结果也与 loading 混为一体。
...
- 设置页与首启共用备份专用模型:WebDAV、百度网盘、FTP、SFTP 的唯一合成桶显示为「备份存储」,但保留原 `BucketInfo.name`、entry id、确认结果和 persisted target。设置页已保存位置的预览按解析后的 storage type 使用同一别名，并精确传回原 bucket + prefix；仅首启或设置页无已保存位置时，才用真实桶名与空 prefix 构造初始目标并自动进入。S3 不套该规则,即使只有一个桶也显示真实桶列表。
...
- **只给选择器行别名,保留设置页路径预览的原始桶名** — 预览同样属于备份入口，打开前暴露用户名会让同一目标前后名称不一致；它应复用 storage-type 的展示规则，同时不能反向影响已保存的 raw target。
...
- WebDAV、百度网盘、FTP、SFTP 的单根在设置页和首启都显示「备份存储」,但桥接调用和固化的目标仍使用真实桶名;S3 继续从真实桶列表选择。已保存且仍有效的目标继续直达原目录，设置页预览也不会泄漏合成桶的后端名字。
...
`test/remote_directory_picker_dialog_test.dart` 覆盖手机/宽屏动作布局、1.5 倍文字缩放、短名称无视觉截断、四种单根 provider 的别名/identity、单桶 S3、设置页已保存路径预览与 raw target round-trip、debug 参数 round-trip、确认结果、无效初始目标、错误恢复、迟到响应隔离与空快照状态。
