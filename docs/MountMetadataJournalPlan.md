...
- **Rename versus writeback:** a synchronous mount rename holds the staging gate, drains queued and running writes for its source path/subtree, then sends the remote move. Cache files are moved before cache indexes and queue records change; a failed physical move leaves the source queue record intact rather than turning the next flush into a false `flush-missing` success.
