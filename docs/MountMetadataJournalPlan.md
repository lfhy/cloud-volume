...
- Local mutations in one namespace extend a shared in-memory quiet barrier after their journal transaction commits. This coalesces recursive Finder copies across many mkdir/write/rename/delete operations, so early entries do not begin provider work while later local children are still arriving. The worker rebuilds the remaining barrier from unsettled journal timestamps after reopen; explicit trigger/retry skips it once, and exit drain skips only this barrier while retaining retry deadlines and dependencies. An open WebDAV write that has not closed has not yet entered the journal and is therefore outside this admission-level barrier.
- Background polling and explicit exit drain serialize complete claim-and-execute passes. This preserves the provider order for a replacement rename's delete-old-target and move-source journal events while still allowing drain to bypass only the namespace quiet barrier; drain uses a nonblocking gate attempt so an in-flight background provider call cannot suppress its cancellation context.
...
- Schema v5 marks a path-facade `ContentRef` as awaiting a journal owner. Failed staging/journal append rolls back a newly allocated inode; startup removes an unowned marked ref and its phantom inode after a crash. Raw low-level `StageWrite` stays available for its explicit caller contract.
...
- **Recorded P2 design gaps:** page task IDs are currently admission IDs rather than worker transfer IDs (namespace-qualified `metadata-op-<namespace>-<seq>` remains the worker snapshot). (2026-08-18) The other gap is closed: pending page-upload chunks are now a mount byte-read source — `metadata.Service.ReadPendingRange`/`CopyPendingContent` serve staged chunks before any provider range read, FUSE/WinFsp materialize pending chunks into the ordinary cache file guarded by an inode+generation stamp with singleflight and post-copy revalidation, and Cloud Files cached reads validate the pending generation before reusing a path-keyed cache file. Remaining durable task ownership work is outside the first MVP closure.
...
- Schema v5 adds `task_groups`/`task_members` and a monotonic journal sequence; `Task` IDs are namespace-qualified (`sync:<namespace>:<group>`) with a source-sequence suffix for mixed lifecycle segments. The projection folds only deterministic, compatible sequences: unconfirmed mkdir plus renames, consecutive pending renames, and older same-inode writes superseded by a newer generation. Raw journal events remain expandable and folded entries retain every namespace-qualified physical `metadata-op-<namespace>-<seq>` reference.
...
- Cancellation is transactional for fully pending groups (Desired edges, inode records, and chunk nlinks roll back together). A provider-running cancel is immediately `cancel_requested`, then `reconciling`; the worker probes frozen provider edges and applies only safe compensating moves/deletes before marking it canceled. Failed/conflict tasks support retry and pending tasks support trigger without bypassing dependencies. Terminal history is scoped and compacted after 30 days without breaking surviving group IDs.
- Flutter `RemoteTaskStore` is the only remote-task display source for transfers, sidebar, file-sync cards, file preview progress, batch progress, mounted sync badges, and app-update status. Dart-only short-lived producers publish through the explicit `source=local` adapter; `TransferQueue` remains an execution/local compatibility producer only. Endpoint failures surface as freshness errors rather than silently restoring the old list.
