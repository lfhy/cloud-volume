...
- Background polling and explicit exit drain serialize complete claim-and-execute passes. This preserves the provider order for a replacement rename's delete-old-target and move-source journal events while still allowing drain to bypass only the namespace quiet barrier.
